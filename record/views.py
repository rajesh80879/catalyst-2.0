...
def upload_data(request):
    if request.method == "POST":
        ...
        if file:
            ...
            try:
                decoded_chunk = file.read().decode("utf-8")
            except UnicodeDecodeError as e:
                messages.error(request, "Error decoding file")
                return redirect("dashboard")

            reader_chunk = csv.reader(io.StringIO(decoded_chunk))
            headers = next(reader_chunk)

            try:
                records_to_create = []
                rows_processed = 0

                for row_values in reader_chunk:
                    row = dict(zip(headers, row_values))

                    name = row.get("name")
                    domain = row.get("domain")
                    year_founded_str = row.get("year founded", "")
                    industry = row.get("industry")
                    size_range = row.get("size range")
                    linkedin_url = row.get("linkedin url")
                    current_employees = (
                        int(row.get("current employee estimate"))
                        if row.get("current employee estimate")
                        else None
                    )
                    total_employees = (
                        int(row.get("total employee estimate"))
                        if row.get("total employee estimate")
                        else None
                    )
                ...
                if records_to_create:
                    ...
                    messages.error(request, "No records found in the file")

            except Exception as e:
                messages.error(
                    request, "Something went wrong while processing the file"
                )
                return redirect("dashboard")
